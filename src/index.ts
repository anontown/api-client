...
export class API {
    ...
    private request<T>(path: string, params: any, token: api.Token | null, authUser: AuthUser | null, recaptcha: string | null): Observable<T> {
        let authToken = this.toAuthToken(token);
        let url = this.config.httpOrigin + name;
        return Observable.fromPromise(fetch(url, {
            ...
            .flatMap(r => Observable.fromPromise(r.json().then(json => ({ status: r.status, json }))))
            .map(r => {
                if (r.status === 200) {
                    return r.json;
                } else {
                    return Observable.throw(new AtError(r.status, r.json.type, r.json.errors))
                }
            });
    }
